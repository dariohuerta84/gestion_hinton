...
| Dockers | Contenido | Función | Puerto | ¿Tiene Jupyter? | Ejecutable (Copiar y Pegar en Terminal) |
| :--- | :--- | :--- | :---: | :---: | :--- |
| **DockerFile** | Python basic | Demostrar el funcionamiento básico de construcción (build) y ejecución (run) de un contenedor que imprime un mensaje de saludo. | No necesita | No | `sudo docker build -t dockerfile-basic -f Dockerfile/Dockerfile Dockerfile && sudo docker run --rm --name docker_basic dockerfile-basic` |
| **DockerFile_MC** | NumPy, Pandas, Scikit-learn, Matplotlib, Seaborn y Jupyter Deep Learning: TensorFlow y PyTorch. | Proporcionar un IDE web interactivo con sincronización bidireccional de archivos mediante volúmenes. | 8888 | Sí | `sudo docker build -t dockerfile_mc -f Dockerfile_MC/Dockerfile Dockerfile_MC && sudo docker rm -f docker_mc || true && sudo docker run -d --name docker_mc -p 8888:8888 -v /home/hinton1/Documents/ADMINISTRACION/repo/gestion_hinton/Dockerfile_MC:/app dockerfile_mc` |
| **DockerFile_MC2** | Pytorch y Numpy. Sistema: build essential. | Ejecutar un pipeline completo de entrenamiento en un contenedor efímero que se auto-elimina al terminar. | No utiliza | No | `sudo docker build -t dockerfile_mc2 -f Dockerfile_MC2/Dockerfile Dockerfile_MC2 && sudo docker run --rm --name docker_mc2_pipeline -v /home/hinton1/Documents/ADMINISTRACION/repo/gestion_hinton/Dockerfile_MC2:/app dockerfile_mc2` |
| **docker_opencv** | opencv-python-headless, numpy, matplotlib, imutils. | Proporcionar un entorno especializado para el procesamiento de imágenes y visión computacional, optimizado para ligereza. | 8889 | Sí | `sudo docker rm -f docker_opencv || true && sudo docker run -d --name docker_opencv -p 8889:8888 -v /home/hinton1/Documents/ADMINISTRACION/repo/gestion_hinton/Dockerfile_OpenCV:/app opencv-jupyter:v1` |
| **DockerFile_SKlearn** | Scikit-learn, Pandas y NumPy. | Validar la instalación de la pila de ML y realizar pruebas de concepto (PoC) como regresiones lineales simples. | 5000 | No | `sudo docker build -t dockerfile_sklearn -f Dockerfile_Sklearn/Dockerfile Dockerfile_Sklearn && sudo docker rm -f docker_sklearn || true && sudo docker run -d --name docker_sklearn -p 5000:5000 -v /home/hinton1/Documents/ADMINISTRACION/repo/gestion_hinton/Dockerfile_Sklearn:/app dockerfile_sklearn` |
| **docker_limited** | Jupyter, NumPy y Pandas. | Entorno interactivo ligero para pruebas rápidas de datos, transformado desde un servidor Nginx. | 8890 | Sí | `sudo docker rm -f docker_limited || true && sudo docker run -d --name docker_limited -p 8890:8888 -v /home/hinton1/Documents/ADMINISTRACION/repo/gestion_hinton/Dockerfile_Limited:/app limited-jupyter:v1` |